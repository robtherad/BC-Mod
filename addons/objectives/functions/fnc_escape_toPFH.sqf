/* ----------------------------------------------------------------------------
Function: bc_objectives_fnc_escape_toPFH
Description:
    Activated by the Objective - Escape module. PFH that runs every 10 seconds to check to see if all synced units are inside the synced trigger.
---------------------------------------------------------------------------- */
...

{
    ...
    if ((count _unitsOutside)/_unitCount > _failPercentage) exitWith {};
} forEach _units;

if ((count _unitsOutside)/_unitCount <= _failPercentage) then {
