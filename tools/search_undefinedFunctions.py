...
def getFunctions(filepath):
    ...
    for func in modfuncs:
        fileFuncs.append("bc_{0}_fnc_{1}".format(selfmodule,func))

    for exModule,func in exfuncs:
        fileFuncs.append("bc_{0}_fnc_{1}".format(exModule, func))
...
def getStrings(filepath):
    ...
    for localString in modStrings:
        fileStrings.append("STR_BC_{0}_{1}".format(selfmodule, localString))

    for (exModule, exString) in exStrings:
        fileStrings.append("STR_BC_{0}_{1}".format(exModule, exString))
